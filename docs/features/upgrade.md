...
```bash
fury tx gov deposit <proposal-id> <deposit> --from=<from> --chain-id=<chain-id> --fees=0.3fury -b block -y
```
...
```bash
fury tx gov vote <proposal-id> <option> --from=<from> --chain-id=<chain-id> --fees=0.3fury -b block -y
```
